...
describe('Role Middleware', () => {
  describe('ROUTE_PERMISSIONS', () => {
    it('deve ter permissões definidas para rotas de clientes', () => {
      ...
      const permissions = ROUTE_PERMISSIONS['/api/clientes'].map(
        p => p.permission
      );
    ...
    it('deve ter permissões definidas para rotas de ordens de serviço', () => {
      ...
      const permissions = ROUTE_PERMISSIONS['/api/ordens-servico'].map(
        p => p.permission
      );
    ...
    it('deve ter permissões definidas para rotas de pagamentos', () => {
      ...
      const permissions = ROUTE_PERMISSIONS['/api/pagamentos'].map(
        p => p.permission
      );
    ...
    it('deve ter permissões definidas para rotas de peças', () => {
      ...
      const permissions = ROUTE_PERMISSIONS['/api/pecas'].map(
        p => p.permission
      );
    ...
    it('deve ter permissões definidas para rotas de relatórios básicos', () => {
      ...
      const permissions = ROUTE_PERMISSIONS['/api/relatorios/basic'].map(
        p => p.permission
      );
    ...
    it('deve ter permissões definidas para rotas de administração', () => {
      ...
      const permissions = ROUTE_PERMISSIONS['/api/admin/users'].map(
        p => p.permission
      );
    ...
    it('deve ter permissões específicas para métodos HTTP diferentes', () => {
      ...
      const readPermissions = allPermissions.filter(p =>
        p.permission.includes('.read')
      );
      const createPermissions = allPermissions.filter(p =>
        p.permission.includes('.create')
      );
      const updatePermissions = allPermissions.filter(p =>
        p.permission.includes('.update')
      );
      const deletePermissions = allPermissions.filter(p =>
        p.permission.includes('.delete')
      );
  ...
  describe('Estrutura de Permissões', () => {
    it('deve ter permissões consistentes para cada módulo', () => {
      ...
      modules.forEach(module => {
        actions.forEach(action => {
          ...
          const hasPermission = Object.values(ROUTE_PERMISSIONS)
            .flat()
            .some(p => p.permission === permission);
    ...
    it('deve ter permissões de admin para rotas administrativas', () => {
      ...
      adminRoutes.forEach(route => {
        const routePermissions = ROUTE_PERMISSIONS[route];
        const hasAdminPermission = routePermissions.some(p =>
          p.permission.includes('admin.')
        );

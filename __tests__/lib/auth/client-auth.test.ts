...
describe('Client Auth Functions', () => {
  describe('generateClientCredentials', () => {
    it('generates credentials with login and senha', () => {
      const credentials = generateClientCredentials('joao@email.com', 'João Silva')
    ...
    it('generates different senhas on multiple calls', () => {
      const credentials1 = generateClientCredentials('test@email.com', 'Test User')
      const credentials2 = generateClientCredentials('test@email.com', 'Test User')
      
      expect(credentials1.senha).not.toBe(credentials2.senha)
    })

    it('handles special characters in email and name', () => {
      const credentials = generateClientCredentials('joão.silva@email.com', 'João da Silva')
    ...
    it('generates senha with valid characters', () => {
      const credentials = generateClientCredentials('test@email.com', 'Test User')
  ...
  describe('Integration Tests', () => {
    it('can hash and verify generated password', async () => {
      const credentials = generateClientCredentials('test@example.com', 'Test User')

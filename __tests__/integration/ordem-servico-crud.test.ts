...
describe('CRUD de Ordens de Serviço - Integração', () => {
  ...
  describe('Listar Ordens de Serviço', () => {
    ...
    it('deve filtrar ordens por status', async () => {
      ...
      const response = await fetch('/api/ordens-servico?status=pendente&page=1&limit=10')
      const data = await response.json()

      expect(fetch).toHaveBeenCalledWith('/api/ordens-servico?status=pendente&page=1&limit=10')
    ...
    it('deve filtrar ordens por cliente', async () => {
      ...
      const response = await fetch('/api/ordens-servico?clienteId=1&page=1&limit=10')
      const data = await response.json()
  ...
  describe('Fluxo completo de Ordem de Serviço', () => {
    it('deve executar ciclo completo da ordem', async () => {
      ...
      ;(fetch as jest.Mock).mockResolvedValueOnce({
        ok: true,
        status: 201,
        json: async () => ({ id: '1', numero: 'OS-2024-001', ...novaOrdem, status: 'pendente' }),
      })
      ...
      ;(fetch as jest.Mock).mockResolvedValueOnce({
        ok: true,
        status: 200,
        json: async () => ({ id: '1', valor: 350.00 }),
      })

      const updateResponse = await fetch('/api/ordens-servico/1', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ valor: 350.00 })
      })

...
describe('WhatsApp Webhook Configuration', () => {
  ...
  describe('Configuração de ambiente', () => {
    test('deve ter token de verificação configurado', () => {
      ...
      expect(process.env.WHATSAPP_WEBHOOK_VERIFY_TOKEN).toBe(
        'test_verify_token'
      );
    });
  });
  ...
  describe('Estrutura de dados do WhatsApp', () => {
    ...
    test('deve validar estrutura de status update', () => {
      ...
      expect(statusPayload.entry[0].changes[0].value.statuses[0].status).toBe(
        'delivered'
      );
    });
  });
  ...
  describe('Validação de dados', () => {
    test('deve validar formato de telefone brasileiro', () => {
      const validPhones = ['5511999999999', '5521888888888', '5531777777777'];

      const invalidPhones = ['11999999999', '999999999', 'invalid'];

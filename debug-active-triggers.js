...
async function debugFunctions() {
  ...
  try {
    ...
    if (normalError) {
      console.log('❌ Erro confirmado na inserção normal:', normalError.message);
      ...
      if (normalError.message.includes('cp.cliente_id')) {
        ...
        console.log('📍 Isso indica que há um trigger ou constraint que executa uma função problemática');
        ...
        console.log('1. trigger_email_ordem_servico (criado em 20250126130000_add_email_triggers.sql)');
        ...
        console.log('📝 Vamos verificar se podemos desabilitar temporariamente este trigger...');
        
        return {
          error: normalError,
          analysis: 'Erro confirmado relacionado a cp.cliente_id em trigger ou função ativa'
        };
      }
    } else {
...
async function main() {
  ...
  if (result.error) {
    ...
    console.log('1. Verificar se há uma versão antiga da função trigger_email_ordem_servico');
    console.log('2. Recriar a função trigger_email_ordem_servico sem referências a cp.cliente_id');
    console.log('3. Verificar se há outros triggers não identificados');
    console.log('4. Considerar desabilitar temporariamente os triggers de email');
  }
}

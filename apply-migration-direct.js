...
async function applyMigrationViaSupabase() {
  console.log('🚀 Iniciando correção do problema cp.cliente_id via Supabase...\n');
  ...
  try {
    ...
    const migrationPath = path.join(__dirname, 'supabase/migrations/20250926090000_force_fix_processar_emails.sql');
    ...
    console.log('🎯 Objetivo: Corrigir função processar_emails_pendentes removendo cp.cliente_id\n');
      ...
      if (command.trim()) {
        ...
        try {
          ...
          if (error) {
            ...
            console.log('⚠️ RPC execute_sql não disponível, tentando abordagem alternativa...');
            break;
          } else {
    ...
    try {
      ...
      if (error) {
        ...
        console.log('❌ ATENÇÃO: A função ainda contém referência a cp.cliente_id');
      } else {
...
async function tryDirectFunctionFix(supabase) {
  console.log('🔧 Tentando correção direta da função processar_emails_pendentes...');
  ...
  try {
    ...
    try {
      const { data, error } = await supabase.rpc('execute_sql', { sql: fixSQL });
      if (!error) {
    ...
    console.log('Como não conseguimos executar SQL diretamente via API, você precisa:');
    console.log('');
    console.log('1. Acessar o Dashboard do Supabase:');
    console.log('   https://supabase.com/dashboard/project/qwbtqlkvooguijchbuxx');
    console.log('');
    ...
    console.log('```');
    console.log('');
    console.log('4. Depois executar o teste novamente com: node test-insert.js');

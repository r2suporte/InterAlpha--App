...
export function OrdemServicoActions({ ordem, onUpdate }: OrdemServicoActionsProps) {
      ...
      {canGenerateSignature() && (
        <Dialog open={showSignatureDialog} onOpenChange={setShowSignatureDialog}>
          <DialogTrigger asChild>
            <Button 
              variant={signatureInfo.status === 'assinado' ? 'outline' : 'default'}
              size="sm"
              className="flex items-center gap-2"
            >
              <FileSignature className="h-4 w-4" />
              {signatureInfo.status === 'assinado' ? 'Ver Assinatura' : 'Gerar Assinatura'}
            </Button>
          </DialogTrigger>

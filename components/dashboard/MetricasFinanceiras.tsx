...
export function MetricasFinanceiras({ className = '' }: MetricasFinanceirasProps) {
  ...
  return (
    <div className={`grid gap-4 md:grid-cols-2 lg:grid-cols-4 ${className}`}>
      {metricas.map((metrica, index) => (
        <Card key={index} className={`transition-all hover:shadow-md ${getCardColor(metrica.cor || 'default')}`}>
          ...
          <CardContent>
            ...
            {metrica.tendencia && (
              <div className={`flex items-center text-xs mt-2 ${getTendenciaColor(metrica.tendencia.tipo)}`}>

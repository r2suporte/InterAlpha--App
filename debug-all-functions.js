  ...
  try {
    ...
    if (funcError) {
      ...
      functions.forEach(func => {
        ...
        if (func.routine_definition && func.routine_definition.includes('cp.cliente_id')) {
    ...
    if (trigError) {
      ...
      triggers.forEach(trigger => {
        ...
        if (trigger.action_statement && trigger.action_statement.includes('cp.cliente_id')) {
          console.log('⚠️  TRIGGER CONTÉM REFERÊNCIA PROBLEMÁTICA: cp.cliente_id');
        }
      });
    }
    ...
    const { data: procFunc, error: procError } = await supabase.rpc('execute_sql', {
      query: `
    ...
    if (procError) {
      console.log('❌ Erro ao verificar função processar_emails_pendentes:', procError);
    ...
    const { data: osTriggers, error: osError } = await supabase.rpc('execute_sql', {
      query: `
        SELECT 
          t.trigger_name,
          t.action_statement,
      ...
      osTriggers.forEach(trigger => {
        ...
        if (trigger.function_source) {
          ...
          if (trigger.function_source.includes('cp.cliente_id')) {
            console.log('⚠️  FUNÇÃO DO TRIGGER CONTÉM REFERÊNCIA PROBLEMÁTICA: cp.cliente_id');
          }
        }
      });
    }

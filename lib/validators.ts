...
export const validarCpfCnpj = (documento: string, tipo: TipoPessoa): boolean => {
...
export const formatarCpfCnpj = (documento: string, tipo: TipoPessoa): string => {
  if (!documento) return ''
...
export const buscarDadosCNPJ = async (cnpjValue: string): Promise<CNPJResponse | null> => {
  try {
    const cleanCnpj = cnpjValue.replace(/\D/g, '')
    ...
    if (!response.ok) {
      if (response.status === 404) {
        return { cnpj: cleanCnpj, nome: '', situacao: 'CNPJ não encontrado', atividade_principal: [], erro: true, message: 'CNPJ não encontrado' }
      }
    ...
    return { cnpj: cnpjValue, nome: '', situacao: 'Erro na consulta', atividade_principal: [], erro: true, message: 'Erro ao consultar CNPJ' }
  }
}

...
async function setupDatabase() {
  try {
    ...
    for (let i = 0; i < sqlCommands.length; i++) {
      ...
      if (command.trim()) {
        try {
          ...
          console.log(`⚠️  Comando ${i + 1} falhou (pode ser normal se já existir):`, error.message.split('\n')[0]);
        }
      }
    }
    ...
    try {
      ...
      console.log('❌ Erro na tabela ordens_servico:', error.message.split('\n')[0]);
    }
    ...
    try {
      ...
      console.log('❌ Erro na tabela pagamentos:', error.message.split('\n')[0]);
    }

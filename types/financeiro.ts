...
// Funções utilitárias
export function calcularValorTotal(itensServico: ItemServico[], itensPecas: ItemPeca[]): number {
  const totalServicos = itensServico.reduce((sum, item) => sum + item.valor_total, 0)
  const totalPecas = itensPecas.reduce((sum, item) => sum + item.valor_total, 0)
...
export function calcularValorPendente(valorTotal: number, pagamentos: Pagamento[]): number {

...
export function determinarStatusEstoque(quantidade: number, estoqueMinimo: number): StatusPeca {
  if (quantidade === 0) return 'sem_estoque'
  if (quantidade <= estoqueMinimo) return 'baixo_estoque'
  return 'disponivel'
}

export function validarPartNumber(partNumber: string): { isValid: boolean; message: string } {
  if (!partNumber || partNumber.trim().length === 0) {
    return { isValid: false, message: 'Part number é obrigatório' }
  }
  
  if (partNumber.length < 3) {
    return { isValid: false, message: 'Part number deve ter pelo menos 3 caracteres' }
  }
  
  if (partNumber.length > 50) {
    return { isValid: false, message: 'Part number não pode ter mais de 50 caracteres' }
  }
  
  // Validação de formato (letras, números, hífen, underscore)
  const partNumberRegex = /^[A-Za-z0-9\-_]+$/
  if (!partNumberRegex.test(partNumber)) {
    return { isValid: false, message: 'Part number deve conter apenas letras, números, hífen e underscore' }
  }

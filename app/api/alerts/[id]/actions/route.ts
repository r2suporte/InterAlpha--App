...
async function performAlertAction(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
      ...
      case 'acknowledge':
        if (!acknowledged_by) {
          return NextResponse.json(
            { 
              success: false, 
              error: 'Campo acknowledged_by é obrigatório para ação acknowledge' 
            },
            { status: 400 }
          );
        }
        success = await alertService.acknowledgeAlert(id, acknowledged_by);
        message = success ? 'Alerta reconhecido com sucesso' : 'Erro ao reconhecer alerta';
        break;

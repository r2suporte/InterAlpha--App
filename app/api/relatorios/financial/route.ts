...
export async function GET(request: NextRequest) {
  try {
    ...
    const ordensFinalizadas = ordens?.filter(ordem => ordem.status === 'finalizada') || []
    const receitaTotal = ordensFinalizadas.reduce((acc, ordem) => acc + (ordem.valor_total || 0), 0)
    const ticketMedio = ordensFinalizadas.length > 0 ? receitaTotal / ordensFinalizadas.length : 0

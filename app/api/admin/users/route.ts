...
import { requireRoles, AuthenticatedUser } from '@/lib/auth/role-middleware'
...
export async function GET(request: NextRequest) {
  const middleware = requireRoles('admin', 'diretor', 'gerente_adm', 'gerente_financeiro')
...
async function createUser(request: NextRequest, user: AuthenticatedUser) {
  try {
    ...
    const allowedRoles = ['user', 'technician', 'atendente', 'supervisor_tecnico', 'gerente_adm', 'gerente_financeiro']
    
    // Apenas admin e diretor podem criar outros diretores
    if (role === 'diretor' && !['admin', 'diretor'].includes(user.role)) {
      return NextResponse.json({ 
        error: 'Apenas administradores e diretores podem criar outros diretores' 
      }, { status: 403 })
    }

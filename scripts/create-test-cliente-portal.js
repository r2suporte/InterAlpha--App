...
async function createTestDataForCliente(cliente) {
  try {
    ...
    let clienteExistente = await supabase
      .from('clientes')
      .select('id')
      .limit(1)
      .single();
    ...
    const ordensData = [
      {
        ...
        descricao: 'Manutenção preventiva completa do sistema de ar condicionado central',
        status: 'em_andamento',
        prioridade: 'media',
        valor_servico: 850.00,
      ...
      },
      {
        ...
        descricao: 'Reparo no quadro elétrico principal - substituição de disjuntores',
        status: 'aguardando_aprovacao',
        prioridade: 'alta',
        valor_servico: 1200.00,
        data_abertura: new Date(Date.now() - 2 * 24 * 60 * 60 * 1000).toISOString(), // 2 dias atrás
      ...
      },
      {
        ...
        data_abertura: new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString(), // 7 dias atrás
        data_inicio: new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString(),
        data_conclusao: new Date(Date.now() - 1 * 24 * 60 * 60 * 1000).toISOString(), // 1 dia atrás
        cliente_portal_id: cliente.id,
    ...
    if (ordensExistentes && ordensExistentes.length > 0) {
      console.log(`✅ ${ordensExistentes.length} ordens de serviço já existem para este cliente`);
    } else {
      ...
      if (errorOrdens) {
        ...
        const ordemAprovacao = novasOrdens.find(o => o.status === 'aguardando_aprovacao');
        if (ordemAprovacao) {

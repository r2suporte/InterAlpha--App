  ...
  try {
    ...
    if (equipamentosError) {
      ...
      equipamentos.forEach((equipamento, index) => {
        console.log(`   ${index + 1}. ${equipamento.tipo} ${equipamento.modelo} - ${equipamento.serial_number}`)
      })
    }
    ...
    if (ordensError) {
      ...
      ordens.forEach((ordem, index) => {
        console.log(`   ${index + 1}. ${ordem.titulo} - Status: ${ordem.status} - Valor: R$ ${ordem.valor_servico}`)
        console.log(`      Cliente ID: ${ordem.cliente_id}, Equipamento ID: ${ordem.equipamento_id}`)

...
async function ensureUserInTable(authUser, userData) {
  try {
    ...
    if (insertError) {
      console.log(
        '⚠️ Erro ao criar registro na tabela users:',
        insertError.message
      );
      return null;
    } 
    ...
    console.error(
      '❌ Erro ao verificar/criar usuário na tabela:',
      error.message
    );
    return null;
  }
...
async function createTestData() {
  try {
    ...
    if (userError || !testUser) {
      console.log(
        '⚠️ Não foi possível encontrar o usuário de teste na tabela users'
      );
      return;
    }
    ...
    if (allClientes && allClientes.length > 0) {
      ...
      if (existingOrdens && existingOrdens.length > 0) {
        ...
        if (ordensError) {
          console.log(
            '⚠️ Erro ao criar ordens de serviço:',
            ordensError.message
          );
        } else {
